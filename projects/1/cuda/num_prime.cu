...
//	printf("gridDim: %d,block.x: %d, block.y: %d, thread.x: %d, thread:y: %d, col: %d, row: %d, i: %d\n",gridDim.x, blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y, col, row, i);
...
int main(int argc, char** argv)
{
	...
	if (argc != 3)
	{
		std::cout << "USAGE: num_prime <number of blocks> <integer>" << std::endl;
		return -1;
	}
	...
	num_blocks = 1000 ;
	int num_thread = n / num_blocks; // use fixed number of thread in each block for convience
	...
	countNumOfPrimerKernel<<< dim3(num_blocks), dim3(num_thread)>>> (n_array_d, is_prime_d);
	...
	std::cout << "Number of primes between 0 and " << n << " is: " << sum_num_prime << std::endl;
